...
export async function PATCH(
  ...
  try {
    ...
    if (!sizeId) {
        return new NextResponse("Ukuran perlu diinput", { status: 400 });
    }

    if (!colorId) {
        return new NextResponse("Warna perlu diinput", { status: 400 });
    }
    if (!params.productId) {
      return new NextResponse("Produk Diinputkan", { status: 400 });
    }
